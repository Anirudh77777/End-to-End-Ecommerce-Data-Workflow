...
class SellerProductSiverETL(TableETL):
    """
    SellerProductSiverETL is a class that extends the TableETL class to perform ETL operations on seller product data.
    It extracts data from upstream ETL tables, transforms the data by adding a new column for actual price and a timestamp
    for ETL insertion, and reads data from the specified storage path.
    Attributes:
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        seller_product_data = upstream_datasets[0].current_data
        current_timestamp = datetime.now()

        tranformed_data = seller_product_data.withColumn(
    ...
    def read(self, partition_values: Optional[Dict[str, str]] = None) -> ETLDataSet:
        ...
        fact_order_data = seller_product_data.select(selected_columns)
