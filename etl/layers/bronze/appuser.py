...
class AppUserBronzeETL(TableETL):
    """
    AppUserBronzeETL is a class that handles the ETL (Extract, Transform, Load) process for the 'appuser' table in the bronze layer of the data lake.
    ...
    def extract_upstream(self) -> List[ETLDataSet]:
        ...
        This method retrieves user data from a specified table in the upstream source,
    ...
    def transform_upstream(
        ...
        """
        user_data = upstream_datasets[0].current_data
        current_timestamp = datetime.now()

        tranformed_data = user_data.withColumn("etl_inserted", lit(current_timestamp))
