...
class BuyerBronzeETL(TableETL):
    """
    BuyerBronzeETL is a class that handles the ETL (Extract, Transform, Load) process for the 'buyer' table in the bronze layer of the data lake.
    ...
    def extract_upstream(self) -> List[ETLDataSet]:
        ...
        This method retrieves buyer data from a specified table in the upstream source,
    ...
    def transform_upstream(
        ...
        """
        buyer_data = upstream_datasets[0].current_data
        current_timestamp = datetime.now()

        tranformed_data = buyer_data.withColumn("etl_inserted", lit(current_timestamp))
