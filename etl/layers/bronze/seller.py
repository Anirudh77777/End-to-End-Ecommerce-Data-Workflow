...
class SellerBronzeETL(TableETL):
    """
    SellerBronzeETL is a class that handles the ETL (Extract, Transform, Load) process for the 'seller' table in the bronze layer of the data lake.
    ...
    def extract_upstream(self) -> List[ETLDataSet]:
        ...
        This method retrieves seller data from a specified table in the upstream source,
        ...
        seller_data = get_upstream_table(table_name, self.spark)

        etl_dataset = ETLDataSet(
            name=self.name,
            current_data=seller_data,
    ...
    def transform_upstream(
        ...
        """
        seller_data = upstream_datasets[0].current_data
        current_timestamp = datetime.now()

        tranformed_data = seller_data.withColumn("etl_inserted", lit(current_timestamp))
