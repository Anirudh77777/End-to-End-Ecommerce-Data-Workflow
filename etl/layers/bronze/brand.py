...
class BrandBronzeETL(TableETL):
    """
    BrandBronzeETL is a class that handles the ETL (Extract, Transform, Load) process for the 'brand' table in the bronze layer of the data lake.
    ...
    def extract_upstream(self) -> List[ETLDataSet]:
        ...
        This method retrieves brand data from a specified table in the upstream source,
        ...
        brand_data = get_upstream_table(table_name, self.spark)
    ...
    def transform_upstream(
        ...
        """
        brand_data = upstream_datasets[0].current_data
        current_timestamp = datetime.now()

        tranformed_data = brand_data.withColumn("etl_inserted", lit(current_timestamp))
