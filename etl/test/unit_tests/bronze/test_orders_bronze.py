...
class TestOrdersBronzeETL:
    """
    Test suite for the OrdersBronzeETL class.
    This test suite contains unit tests for the methods of the OrdersBronzeETL class, which is responsible for
    extracting and transforming data in the ETL pipeline.
    Classes:
        TestOrdersBronzeETL: Contains unit tests for the OrdersBronzeETL class.
    Methods:
    ...
    def test_extract_upstream(self, spark):
        """
        Test the extract_upstream method of the OrdersBronzeETL class.
        This test initializes an instance of the OrdersBronzeETL class with a Spark session,
        calls the extract_upstream method, and asserts that the first    element in the resulting
        dataset has a name attribute equal to "orders".
        Args:
        ...
        orders_table = OrdersBronzeETL(spark=spark)
        orders_table_dataset = orders_table.extract_upstream()

        assert orders_table_dataset[0].name == "orders"

    def test_transform_upstream(self, spark):
        """
        Test the `transform_upstream` method of the `OrdersBronzeETL` class.
        ...
        2. Instantiate the `OrdersBronzeETL` class.
        ...
        upstream_dataset = ETLDataSet(
            ...
            primary_keys=orders_table.primary_keys,
            storage_path=orders_table.storage_path,
            data_format=orders_table.data_format,
            database=orders_table.database,
            partition_keys=orders_table.partition_keys,
        )

        transformed_datasets = orders_table.transform_upstream([upstream_dataset])
