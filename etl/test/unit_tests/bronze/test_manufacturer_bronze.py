...
class TestManufacturerBronzeETL:
    """
    Test suite for the ManufacturerBronzeETL class.
    This test suite contains unit tests for the methods of the ManufacturerBronzeETL class, which is responsible for
    extracting and transforming data in the ETL pipeline.
    Classes:
        TestManufacturerBronzeETL: Contains unit tests for the ManufacturerBronzeETL class.
    Methods:
    ...
    def test_extract_upstream(self, spark):
        """
        Test the extract_upstream method of the ManufacturerBronzeETL class.
        This test initializes an instance of the ManufacturerBronzeETL class with a Spark session,
        calls the extract_upstream method, and asserts that the first    element in the resulting
        dataset has a name attribute equal to "manufacturer".
        Args:
        ...
        Asserts:
            ...
            is equal to "manufacturer".
        """

        manufacturer_table = ManufacturerBronzeETL(spark=spark)
        manufacturer_table_dataset = manufacturer_table.extract_upstream()

        assert manufacturer_table_dataset[0].name == "manufacturer"

    def test_transform_upstream(self, spark):
        """
        Test the `transform_upstream` method of the `ManufacturerBronzeETL` class.
        ...
        2. Instantiate the `ManufacturerBronzeETL` class.
        ...
        upstream_dataset = ETLDataSet(
            name=manufacturer_table.name,
            current_data=upstream_df,
            primary_keys=manufacturer_table.primary_keys,
            storage_path=manufacturer_table.storage_path,
            data_format=manufacturer_table.data_format,
            database=manufacturer_table.database,
            partition_keys=manufacturer_table.partition_keys,
        )

        transformed_datasets = manufacturer_table.transform_upstream([upstream_dataset])
